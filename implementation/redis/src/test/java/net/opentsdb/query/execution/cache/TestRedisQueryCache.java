...
public class TestRedisQueryCache {
  ...
  public void before() throws Exception {
    ...
    // Initialize connection_pool to a pre-existing mock for tests that use it as a
    // shared object (e.g. initializeSharedAlreadyThere). For tests that call
    // initialize(), the MockedConstruction initializer overwrites this with the
    // newly constructed mock.
    connection_pool = mock(JedisPool.class);
    when(connection_pool.getResource()).thenReturn(instance);

    mockedJedisPool = Mockito.mockConstruction(JedisPool.class, (mock, context) -> {
  ...
  public void cache() throws Exception {
    ...
    verify(instance, times(2)).set(key, data, SetParams.setParams().nx().px(600000L));
    verify(connection_pool, never()).close();
