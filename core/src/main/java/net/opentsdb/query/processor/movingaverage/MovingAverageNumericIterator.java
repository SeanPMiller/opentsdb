 ...
 * @since 3.0
 */
public class MovingAverageNumericIterator implements QueryIterator, 
  ...
  /** An array of timestamps to use for windowing. */
   ...
   */
  public MovingAverageNumericIterator(final QueryNode node, 
   ...
   */
  private boolean addValue(final TimeSeriesValue<NumericType> value) {
    if ((long_values != null && value_idx >= long_values.length) ||
        (double_values != null && value_idx >= double_values.length)) {
      if (offset >= 16) {
        // shift!
  ...
  private String arraysToString() {
    ...
    for (int i = offset; i < value_idx; i++) {
      ...
      if (long_values != null) {
        buf.append(long_values[i]);
      } else {
        buf.append(double_values[i]);
      }
    }
    buf.append("]");
    return buf.toString();
