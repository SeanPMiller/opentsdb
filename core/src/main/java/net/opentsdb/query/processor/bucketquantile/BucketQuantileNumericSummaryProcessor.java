...
/**
 * Computes percentiles on {@link NumericSummaryType} values.
 * Notes:
...
public class BucketQuantileNumericSummaryProcessor extends BucketQuantileProcessor {
  ...
  /** The summary we'll work with. */
  protected int summary = -1;
  ...
  void run() {
    ...
    try {
      ...
      final TimeSeriesValue<NumericSummaryType>[] values = 
      ...
      for (int i = 0; i < iterators.length; i++) {
        while (iterators[i] != null && iterators[i].hasNext()) {
          values[i] = (TimeSeriesValue<NumericSummaryType>) iterators[i].next();
          ...
          // TODO we should only have one summary here EVER! So we'll grab the
          // first one that comes in. We may want to verify the other's don't 
          // have different summaries.
          if (summary < 0) {
            final Collection<Integer> summaries = values[i].value().summariesAvailable();
            if (summaries.isEmpty()) {
              LOG.warn("Empty summaries available shouldn't happen.");
              continue;
            }
      ...
      while (true) {
        ...
        for (int i = 0; i < sources.length; i++) {
          ...
          if (values[i].timestamp().compare(Op.EQ, current_ts)) {
            final NumericType value = values[i].value().value(summary);
            if (value.isInteger()) {
  ...
  TimeSeries getSeries(final int percentile_index) {
    return new BucketQuantileNumericSummaryIterator(
        percentile_index, this);
  }
